...
import com.feng.com.rxjavade.utils.JsonUtil;
import com.feng.com.rxjavade.utils.ZipHelper;
...
public class RequestInterceptor implements Interceptor {
    ...
    public RequestInterceptor(HttpHandler handler) {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (mHandler != null)//这里可以比客户端提前一步拿到服务器返回的结果,可以做一些操作,比如token超时,重新获取
            return mHandler.onResponse(bodyString, chain, originalResponse);
    ...
    private String printResult(Request request, Response originalResponse) throws IOException {
        ...
        if (isParseable(responseBody)) {
            ...
            Timber.tag(getTag(request, "Response_Result")).w(isJson(responseBody) ? JsonUtil.jsonFormat(bodyString) : bodyString);
